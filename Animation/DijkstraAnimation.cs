...
namespace DoThi.Animation
{
    public class DijkstraAnimation : UserControl
    {
        ...
        private readonly Pen _highlightedEdgePen = new(Color.Green, 5);
        private readonly Brush _highlightedVertexBrush = Brushes.LightGreen;

        public DijkstraAnimation(List<Vertex> vertices, List<Edge> edges, List<int[]> steps, int sourceVertex)
        {
            _vertices = vertices;
            _edges = edges;
            _steps = steps;
        ...
        private void DrawVertices(Graphics g)
        {
            ...
            {
                var vertexBrush = (_currentStepIndex < _steps.Count && _steps[_currentStepIndex][i] != int.MaxValue)
                    ? _highlightedVertexBrush
                    : _vertexBrush;

                g.FillEllipse(vertexBrush, _vertices[i].Location.X - VertexRadius, _vertices[i].Location.Y - VertexRadius, 2 * VertexRadius, 2 * VertexRadius);
                g.DrawEllipse(_vertexOutlinePen, _vertices[i].Location.X - VertexRadius, _vertices[i].Location.Y - VertexRadius, 2 * VertexRadius, 2 * VertexRadius);

                var stringSize = g.MeasureString(_vertices[i].Value, Font);
        ...
        private void DrawEdges(Graphics g)
        {
            foreach (var edge in _edges)
            {
                ...
                if (_currentStepIndex < _steps.Count &&
                    _steps[_currentStepIndex][edge.Vertex1] != int.MaxValue &&
                    _steps[_currentStepIndex][edge.Vertex2] != int.MaxValue)
                {
                    pen = _highlightedEdgePen;
                }
